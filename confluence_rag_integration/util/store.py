...
class PostgresByteStore(BaseStore):
    ...
    def get(self, key):
        with self.Session() as session:
            # First try to get with default filename
            result = session.execute(select(ByteStore).filter_by(collection_name=self.collection_name, key=key, filename="default")).scalar()
            if not result:
                # If not found with default filename, try any filename
    ...
    def mset(self, items):
        with self.Session() as session:
            for item in items:
                if len(item) == 2:
                    # Standard LangChain interface: (key, value)
                    key, value = item
                    filename = "default"  # Use default filename when not provided
                elif len(item) == 3:
                    # Extended interface: (key, value, filename)
                    key, value, filename = item
                else:
                    raise ValueError(f"Expected 2 or 3 values, got {len(item)}")
